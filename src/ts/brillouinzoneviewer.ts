...
export class BrillouinZoneViewer extends Viewer {
    ...
    createBrillouinZone(
        ...
        for (let iVertex=0; iVertex < vertices.length; ++iVertex) {
            let vertex:number[] = vertices[iVertex];
        ...
        for (let iFace=0; iFace<faces.length; ++iFace) {
            ...
            let nPoints:number = faceIndices.length
            for (let iPoint=0; iPoint<nPoints; ++iPoint) {
                ...
                if (!faceMap.hasOwnProperty(key1.toString()) && !faceMap.hasOwnProperty(key2.toString())) {
                    ...
                    lineGeometry.vertices.push(
                        new THREE.Vector3().fromArray(vertices[firstIndex]).multiplyScalar(1E-10),
                        new THREE.Vector3().fromArray(vertices[secondIndex]).multiplyScalar(1E-10)
                    );
                ...
                if (iPoint < nPoints-2) {
                    let face = new THREE.Face3(
                        faceIndices[0],
                        faceIndices[(iPoint+1) % nPoints],
                        faceIndices[(iPoint+2) % nPoints]
                        //normal
                    )
